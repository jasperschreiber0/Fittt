  ...
  return Response.json(
    {
      ...
      ...Object.fromEntries(tables.map((t, i) => [t, results[i].data])),
    },
...
export async function POST(req: Request) {
  try {
    ...
    const day = body.day ? z.iso.date().parse(body.day) : today;
    if (
      body.day &&
      body.action !== "event" &&
      (day > today || dayDiff(today, day) > 365)
    )
      throw Error("Choose a valid recent date");
    let result;
    switch (body.action) {
      case "profile": {
        ...
        data.calorieLow = t.low;
        data.calorieHigh = t.high;
        result = await s.from("fittt_profiles").upsert({ ...owner, data });
        break;
      }
      case "entry": {
      ...
      case "deleteEntry":
        ...
        break;
      case "day": {
        const data = fastSchema.parse(body.data);
        const { data: entries } = await s
          .from("fittt_entries")
          .select("*")
          .eq("day", day);
      ...
      case "challenge":
        result = await s.rpc("fittt_challenge", {
          action: z.enum(["create", "join"]).parse(body.mode),
          title: body.name ? z.string().min(1).max(60).parse(body.name) : null,
          starts: body.start ? z.iso.date().parse(body.start) : null,
          invite: body.code ? z.string().max(40).parse(body.code) : null,
        });
        break;
      case "leaderboard":
        result = await s.rpc("fittt_leaderboard", {
          cid: z.uuid().parse(body.id),
        });
        break;
      case "deleteDiary":
        for (const t of [
          ...
          const r = await s
            ...
            .delete()
            .eq("user_id", user.id);
          if (r.error) throw Error("Could not clear all private data");
        }
